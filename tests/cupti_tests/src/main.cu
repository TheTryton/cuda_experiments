...
VectorAdd( const int *pA, const int *pB, int *pC, int N)
{
    ...
    {
        int a, b, c;
        memcpyEfficient<sizeof(int)>(&a, pA + i);
        memcpyEfficient<sizeof(int)>(&b, pB + i);
        c = a + b;
        memcpyEfficient<sizeof(int)>(pC + i, &c);
    }
}
...
main(int argc, char *argv[])
{
    ...
    if (metrics.empty())
    {
        metrics.push_back("smsp__sass_sectors_mem_global");
        metrics.push_back("smsp__sass_sectors_mem_global_ideal");
    }
